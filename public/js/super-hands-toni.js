...
AFRAME.registerComponent('super-hands', {
  START_BUTTONS: {
    ...
    'pointup': 'point',
    'thumbup': 'thumb',
    'pointingstart': 'pointing',
    'pistolstart': 'pistol',
    'thumbstickdown': 'thumbstick',
    'mousedown': 'mouse',
    'touchstart': 'touch'
  },
  END_BUTTONS: {
    ...
    'pointdown': 'point',
    'thumbdown': 'thumb',
    'pointingend': 'pointing',
    'pistolend': 'pistol',
    'thumbstickup': 'thumbstick',
    'mouseup': 'mouse',
    'touchend': 'touch'
  },
  schema: {
  ...
  init: function () {
    ...
    // current buttons used
    this.grabButtons = [];
    this.stretchButtons = [];
    this.dragDropButtons = [];
  ...
  onGrabStartButton: function (evt) {
    // We store the button currently pressed
    const btn = this.START_BUTTONS[evt.type];
    const idx = this.grabButtons.indexOf(btn);
    if (idx === -1) {
      this.grabButtons.push(btn);
    }

    console.log("GRAB START", this.grabButtons.join());
  ...
  onGrabEndButton: function (evt) {
    // We remove the button that was pressed
    const btn = this.END_BUTTONS[evt.type];
    const idx = this.grabButtons.indexOf(btn);
    if (idx !== -1) {
      this.grabButtons.splice(idx, 1);
    }
    console.log("GRAB END", this.grabButtons.join());

    // If there's still any button pressed,
    if (this.grabButtons.length > 0) {
      // We do not continue: abort the end grab.
      return;
    }
  ...
  onStretchStartButton: function (evt) {
    // We store the button currently pressed
    const btn = this.START_BUTTONS[evt.type];
    const idx = this.stretchButtons.indexOf(btn);
    if (idx === -1) {
      this.stretchButtons.push(btn);
    }
  ...
  onStretchEndButton: function (evt) {
    ...
    const idx = this.stretchButtons.indexOf(btn);
    if (idx !== -1) {
      this.stretchButtons.splice(idx, 1);
    }
  ...
  onDragDropStartButton: function (evt) {
    // We store the button currently pressed
    const btn = this.START_BUTTONS[evt.type];
    const idx = this.dragDropButtons.indexOf(btn);
    if (idx === -1) {
      this.dragDropButtons.push(btn);
    }
  ...
  onDragDropEndButton: function (evt) {
    ...
    const idx = this.dragDropButtons.indexOf(btn);
    if (idx !== -1) {
      this.dragDropButtons.splice(idx, 1);
    }
