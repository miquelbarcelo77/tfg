  ...
  document.querySelector('a-scene').addEventListener('loaded', function() {
        ...
        const interval = setInterval(() => {
          ...
          if (newWidth >= 0.2) {
            ...
            pieces.forEach((piece) => {
              let material = piece.getAttribute('material');
              material.opacity = Math.min(material.opacity + 0.5, 1); // Aumenta la opacidad, asegurando que no exceda 1
    ...
    AFRAME.registerComponent('bandeja-checker', {
      ...
      handleCollision: function (evt) {
        ...
        if (isCooked) {
          console.log("Objeto cocinado colisionando", collidedEl);
        ...
        .then(data => {
          ...
          data.forEach((ingrediente, index) => {
            ...
            if (!ingrediente.EsTallat && entity) {
              ...
              const progressBarPosition = `${positionX} ${parseFloat(position.split(' ')[1]) - 0.15} -3`; // Posición Y ligeramente arriba del ingrediente
